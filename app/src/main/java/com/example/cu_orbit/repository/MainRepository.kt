...
class MainRepository {
    ...
    suspend fun createChannel(name: String, isPrivate: Boolean, description: String, userId: String? = null): Channel {
        // Find default/active workspace for backward compatibility
        val wsList = getWorkspaces()
        val wsId = if (wsList.isNotEmpty()) wsList[0].id else "" 
        return api.createWorkspaceChannel(wsId, ChannelRequest(name, isPrivate, description, userId))
    }
    ...
    suspend fun createWorkspaceChannel(workspaceId: String, name: String, isPrivate: Boolean, description: String, userId: String? = null) =
        api.createWorkspaceChannel(workspaceId, ChannelRequest(name, isPrivate, description, userId))

    suspend fun getChannel(id: String): Channel = api.getChannel(id)

    suspend fun addChannelMember(channelId: String, phone: String) =
        api.addChannelMember(channelId, mapOf("userId" to phone))
